...
state/               volatile runtime signals; gitignored
  ...
  voice-replies/     one reply record per corr-tagged external turn, key <corr>; written by fm-reply-capture-claude.sh at Stop (Claude only; see its header for the record format and compatibility limits); bin/fm-reply-capture-lib.sh
...
For PR-based ship tasks, the ready signal depends on mode: `no-mistakes` reports `done: PR <url> checks green` followed by `paused: awaiting captain merge decision on PR <url>` after CI is green, while `direct-PR` reports `done: PR <url>` after opening the PR.
