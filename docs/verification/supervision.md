...
agy is an unverified adapter with no primary supervision transport or turn-end hook, so it is absent from this primary table and refused by normal dispatch for every kind ([`references/harness/agy.md`](../../.agents/skills/harness-adapters/references/harness/agy.md)).
