...
| `fm-brief.sh`            | Scaffold ship, scout, spec, plan, secondmate-charter, and Herdr-lab briefs           |
...
| `fm-agy-lib.sh`          | Experimental AGY headless-crewmate adapter: detect, launch, observe, collect, validate, and clean up |
...
| `fm-voice-progress.sh`   | Construct/parse the voice-turn marker and emit silent-phase progress events to the bridge side channel |
...
| `fm-promote.sh`          | Promote a scout, spec, or plan task in place to a protected ship task                |
...
| `fm-reply-capture-claude.sh` | Claude Stop-hook adapter that writes a corr-tagged voice reply record (Claude only; see its header for the record format and compatibility limits) |
| `fm-reply-capture-lib.sh` | Write firstmate's conversational reply for a corr-tagged external turn to `state/voice-replies/<corr>`; owns the corr-token contract reuse and atomic record write (see its header) |
