...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, and scout, spec, and plan reports.
...
The same bootstrap run emits `SECONDMATE_LIVENESS:` when a registered secondmate is skipped, its relaunch fails, or a registry-meta cross-reference mismatch is found; already-live and successfully relaunched secondmates are handled silently.
