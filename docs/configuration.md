...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout, spec, and plan reports, and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
...
agy is experimental and unverified: it is not in the verified adapter list and is refused for secondmate and non-tmux launches until its proof gate passes ([`.agents/skills/harness-adapters/references/harness/agy.md`](../.agents/skills/harness-adapters/references/harness/agy.md) and [runtime backend verification](verification/runtime-backends.md#agy) own the contract and evidence).
