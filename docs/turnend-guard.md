...
- Claude registers three `Stop` hooks in `.claude/settings.json`, all anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`; `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`; and `bin/fm-reply-capture-claude.sh`, which writes a corr-tagged voice reply record at Stop (Claude only; see its header for the record format and compatibility limits).
