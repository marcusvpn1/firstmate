...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, the AFK boundary's live-daemon inertness and dead-daemon detect-and-wake rewake, the unchanged need boundary, single-flight, and exit-2 translation.
