...
install_integrated_autoarm() {
  ...
  chmod +x "$dir/bin/fm-claude-stop-autoarm.sh" "$dir/bin/fm-lock.sh" "$dir/bin/fm-afk-start.sh"
...
test_hook_away_daemon_allows_beacon_within_poll_derived_grace() {
  ...
  beat=$(( $(date +%s) - 400 ))
  if [ "$(uname)" = Darwin ]; then touch -mt "$(date -r "$beat" '+%Y%m%d%H%M.%S')" "$dir/state/.last-watcher-beat"; else touch -d "@$beat" "$dir/state/.last-watcher-beat"; fi
