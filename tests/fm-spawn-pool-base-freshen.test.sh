...
test_scout_refreshes_before_launch() {
  ...
  status=$?
  expect_code 0 "$status" "scout spawn should refresh a stale pooled worktree"
  current=$(git -C "$POOL_DIR" rev-parse origin/main)
  [ "$(git -C "$POOL_DIR" rev-parse HEAD)" = "$current" ] \
    || fail "scout spawn did not start at current origin/main"
  assert_grep 'must survive a newly spawned branch' "$POOL_DIR/advanced-main.txt" \
    "scout spawn omitted advanced-main content"
  if [ "${FM_TEST_EVIDENCE:-0}" = 1 ]; then
    printf '# observed scout spawn: %s\n' "$(printf '%s\n' "$out" | tail -n 1)"
  fi
  pass "scout spawn refreshes a stale pooled worktree before launch"
}
