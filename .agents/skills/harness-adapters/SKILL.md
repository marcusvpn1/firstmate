...
Nothing is lost across the gap (the durable wake queue and `fm-wake-drain.sh` recover it, per Reliability properties in the `/afk` skill). The Stop-hook auto-arm (`bin/fm-claude-stop-autoarm.sh`) detects a dead daemon on its next Stop and rewakes the model (exit 2) so away mode is not silently unsupervised: confirm `state/.afk` is still present, then rerun `FM_AFK_STATE_PREPARED=1 bin/fm-afk-start.sh` through the harness's native background-task tool.
...
The auto-arm hook's detect-and-wake now covers liveness, but a full unattended relaunch from the hook (rather than the model re-running the native task) would need the terminal-backed launch path (`fm-afk-launch.sh start`) and remains unscoped follow-up work, not a small change.
